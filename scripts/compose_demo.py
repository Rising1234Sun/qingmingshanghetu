"""两分钟功能演示配乐：48 小节，沿用原创主题并重新编配发展段与收束。"""
...
ROOT=Path(__file__).resolve().parents[1];SR=48000;DURATION=120;N=SR*DURATION
RNG=np.random.default_rng(1127);BEAT=60/96;BAR=4*BEAT
OUT=ROOT/'output/audio/两分钟功能演示';OUT.mkdir(parents=True,exist_ok=True)
CACHE={};NOTES=[]
...
def main():
    ...
    for bar in range(48):
        t=bar*BAR;chord=chords[(bar//2)%4];energy=.28 if bar<3 else .42 if bar<12 else .50 if bar<24 else .55 if bar<34 else .73 if bar<43 else .45
        if bar==47:chord=chords[0];energy=.35
        ...
        if bar<46 and bar%8 not in [6,7]:
            off=0
            for n,d in themes[bar%4]:
                if 24<=bar<32 and n in [78,81]:n-=2 if n==78 else 3
        ...
        if bar in [0,12,24,34,43]:event(gong,48,t,4,.25 if bar==0 else .36,'鼓与转场',-.1)
        if bar in [11,23,33]:
            a=cymbal.play(0,2.2,.25)[::-1].copy();place(layers['鼓与转场'],a,t+.25,1,pan=.1)
        print('已编配小节',bar+1,flush=True)
    event(harp,74,117.5,2.0,.35,'丝竹旋律',.1)
    # 房间感与尾部自然收束。
    ...
    (ROOT/'docs/两分钟音频测量.json').write_text(json.dumps(reports,ensure_ascii=False,indent=2))
