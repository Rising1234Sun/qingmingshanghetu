...
Git 仓库包含完整网页运行资产与制作脚本。数 GB 原始录像、约 505 MiB 的 4K 成片、`.blend`、渲染序列、软件安装包与采样 WAV 不纳入普通 Git 历史；本地原文件继续保留。可通过 [重建指南](docs/重建指南.md) 生成 Blender 工程，通过采样清单恢复音乐依赖，再录制自己的网页操作。原始录像属于实际操作结果，无法只靠源码自动重现每一次鼠标输入。
